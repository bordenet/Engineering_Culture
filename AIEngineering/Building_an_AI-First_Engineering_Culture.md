...
I practice what I preach. I've shipped a half-dozen fast, disposable internal tools that the engineering and business teams actually use, now consolidated into [DocForge AI](https://bordenet.github.io/docforge-ai/): [One-Pager](https://bordenet.github.io/docforge-ai/assistant/?type=one-pager), [PR-FAQ](https://bordenet.github.io/docforge-ai/assistant/?type=pr-faq), [ADR](https://bordenet.github.io/docforge-ai/assistant/?type=adr), [Strategic Proposal](https://bordenet.github.io/docforge-ai/assistant/?type=strategic-proposal), and [Power Statement](https://bordenet.github.io/docforge-ai/assistant/?type=power-statement). Through near brute-force continuous improvement, the codebase is now reliable enough for regular team use. But it started scrappy, and that was fine. They removed toil from day one.

The path wasn't smooth. I started with One-Pager. Medium-to-low code quality, but it solved an immediate problem. Then I tried to expand by telling Claude Code to clone and mutate One-Pager into a sibling tool responsible for creating a different type of document with a similar "adversarial" multi-LLM workflow. That effort failed because the generated code varied too widely in design, test coverage, and user experience. So I invested in a common core: a template system that gives AI assistants guardrails to work within. That approach eventually evolved into [DocForge AI](https://github.com/bordenet/docforge-ai), which replaced the multi-repo architecture with a single plugin system. Today, spinning up a new document type takes hours, not days, and the quality is consistent. That early YOLO phase taught me what the foundation needed to be.
...
Individual experiments start the process. Mechanisms are how you scale.
...
The financial case matters. A strong core team remains indispensable. But experienced leaders need to confront a reality that would have seemed wasteful five years ago: expensive AI tools often cost less than the headcount they offset.
...
The old calculus was: tools are cheap, people are expensive, so hire more people. The new calculus is: build a smaller, stronger core team and give them the AI capability to operate at a higher level of output. The 12-person team that outpaced 70 reflected changed economics. It was this math in action.
...
The real advantage of AI-First isn't any single tool or technique. It's that each improvement makes the next one easier.

Each iteration makes the team smarter. The [prompt library gets better](https://github.com/bordenet/superpowers-plus/blob/main/skills/observability/evolution-loop/skill.md). The shared knowledge grows. New hires onboard faster because there's infrastructure to support them. In my experience, the gap between your team and teams that haven't made this shift widens over time.
...
- [The One-Pager](../SDLC/The_One-Pager.md) - *Document format used in DocForge AI*
