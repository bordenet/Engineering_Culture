...
Run an anonymous survey. Ask: *How often do you use AI tools? For what tasks?* When I did this, over half our engineers were stuck at Microtasker while a handful were already at Delegate. The gap was wider than I expected—and closing it mattered more than accelerating the high performers.

I now explicitly hire for AI fluency. Not "AI expertise"—fluency. Can candidates describe how they use AI in their current work? Do they reach for prompting or default to manual approaches? This has become a critical indicator of growth mindset for me. Not everyone will agree, but I've stopped apologizing for it.
...
**Week 1: Automate PR descriptions.** This is the benevolent gateway drug. Engineers see immediate value, and the output is low-stakes enough that mistakes don't have a large blast radius. When someone's PR description is better than anything they'd have written manually, they start wondering what else AI can do. And by standardizing commit messages with team templates, you seed the path to release notes, change logs, and other documentation.

**Week 2: Commit messages.** Same pattern. Low risk, high repetition, visible improvement.
...
If an engineer can spend a few hours building a scrappy internal tool that eliminates daily toil for the team—YOLO that thing. Ship it. Yes, the code quality might be questionable. Yes, AI probably generated 80% of it. Yes, it'll need to be rewritten eventually.
...
I practice what I preach. I've shipped a half-dozen YOLO tools that our engineering and business teams actually use: [one-pager](https://bordenet.github.io/one-pager/), [pr-faq-assistant](https://bordenet.github.io/pr-faq-assistant/), [architecture-decision-record](https://bordenet.github.io/architecture-decision-record/), [strategic-proposal](https://bordenet.github.io/strategic-proposal/), [power-statement-assistant](https://bordenet.github.io/power-statement-assistant/), and a [pr-faq-validator](https://github.com/bordenet/pr-faq-validator) CLI. Through near brute-force continuous improvement, the code quality no longer sucks—but it started scrappy, and that was fine. They removed toil from day one.

The path wasn't smooth. I started with [one-pager](https://bordenet.github.io/one-pager/)—medium-to-low code quality, but it worked. Then I tried to expand: told Claude Code to clone and mutate it into sibling tools. That fell flat on its face. Wild variance in code quality, design choices, test coverage, UX decisions. It was laughable. So I invested in a common core—[genesis](https://github.com/bordenet/genesis)—a template system that gives AI assistants guardrails to work within. Now spinning up a new tool takes hours, not days, and the output is consistent. The YOLO phase taught me what the foundation needed to be.
...
**Measure outcomes, not activities.** Skip the OKRs about "AI adoption percentage." Results speak for themselves. If the team is shipping faster, if documentation is better, if engineers are spending less time on toil—you'll see it. If you're not seeing it, no OKR will fix that.

**Leadership models the behavior.** If directors and VPs use AI in their own work—drafting roadmaps, preparing board materials, analyzing metrics—it signals that this isn't just for individual contributors. I started sharing my Claude conversations in leadership meetings. "Here's how I drafted the quarterly planning doc." That changed things faster than any training session.

## The Math Has Changed

Let's talk about money. We love people. We need people. A strong core team is irreplaceable. But experienced leaders need to confront a reality that would have seemed wasteful five years ago: expensive AI tools are often the right investment now.
...
Here's what nobody tells you about AI-First transformation: it compounds.
...
At Telepathy.ai, we didn't have months to rebuild after the LLM revolution gutted our proprietary stack. We threw our existing SaaS platform out the window and built an LLM-native solution with LangGraph. Classical architecture would have taken quarters. Instead, we spent a few weeks on architecture and design, then had end-to-end scenarios running after three weeks of implementation—complete with healthy abstractions. That's not cutting corners. That's a team operating AI-first under existential pressure.
...
I'm now applying these principles at my current company. The transformation is actively in progress—I'll report back in a future post with concrete metrics.
